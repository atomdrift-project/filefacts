...
fn open_archive(
    ...
    // Compressed wrappers. metaparse doesn't take a dep on the compressor
    // crates yet; for now we report the format label and decline to walk
    // entries when the bytes are compressed. (Cleave can hand metaparse
        ...
        return Err(Error::malformed(
            ...
            format!(
                "compressed tar variants are reported by format only; \
                 decompress the wrapper before passing to metaparse (variant={file_type:?})"
            ),
        ));
    }
